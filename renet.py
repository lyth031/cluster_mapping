...
class Array(object):
    ...
    # Constructor
    def __init__(self,nwl = 32, nbl = 32, rlrs = 50e3, rhrs = 500e3, rwl = 3.9, rbl = 3.9, device = "cpu"):
        self.nwl = nwl
        self.nbl = nbl
        self.rlrs = rlrs
        self.rhrs = rhrs
        ...
        self.rbl = rbl
        self.device = device
        self.vnode = torch.zeros([2,self.nwl,self.nbl], device = self.device, dtype=torch.float)
        self.rram = torch.zeros([self.nwl,self.nbl], device = self.device, dtype=torch.float)
    ...
    def realOutput(self,vin):
        ...
        left = 2 / self.rwl * torch.ones([self.nwl, self.nbl-1], device = self.device)
        right = 1 / self.rwl * torch.ones([self.nwl, 1], device = self.device)       
        parasitic_rwl = torch.cat((left, right),1) +  1 / self.rram
        # print(parasitic_rwl)
        top = 1 / self.rbl * torch.ones([1, self.nbl], device = self.device)
        bottom = 2 / self.rbl * torch.ones([self.nwl-1, self.nbl], device = self.device)
        parasitic_rbl = torch.cat((top, bottom), 0) + 1 / self.rram
        # print(parasitic_rbl)
        self.vnode[0] = torch.mm(vin.t(), torch.ones([1,self.nbl], device = self.device, dtype=torch.float))
        ...
        while error > tol and count <= max_step:
            ...
            relax = (torch.div(relax, parasitic_rwl) - self.vnode[0])
            self.vnode[0] += relax
            m = relax.abs().max()
            if error <= m:
                error = m
            ...
            relax = (torch.div(relax, parasitic_rbl) - self.vnode[1])
            self.vnode[1] += relax
            m = relax.abs().max()
            if error <= m:
                error = m
